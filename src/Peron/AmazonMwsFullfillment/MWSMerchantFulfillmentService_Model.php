...
/* use Peron\AmazonMwsFullfillment\AmazonFullfillment;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Client;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Exception;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Interface;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Mock;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Address;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_CreateShipmentRequest;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_CreateShipmentResponse;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_CreateShipmentResult;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Model_FileContents;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Model_GetEligibleShippingServicesRequest;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Model_GetEligibleShippingServicesResponse;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Model_GetEligibleShippingServicesResult;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Model_GetShippingRequest;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Model_GetShippingServicesResponse;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Model_GetShippingResult;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Model_Item;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_Model_Label;
 use Peron\AmazonMwsFullfillment\MWSMerchantFulfillmentService_ShippingService;*/
            ...
            if (is_array($fieldType)) {
                if ($fieldType[0] == "object") {
                    ...
                    if ($elements->length >= 1) {
                        ...
                        foreach ($elements as $element) {
                            if (strpos ($fieldType[0], "Peron\\AmazonMwsFullfillment\\") === false)
                                $fieldType[0] = "Peron\\AmazonMwsFullfillment\\".$fieldType[0];
                ...
                if ($this->_isComplexType($fieldType)) {
                    ...
                    if ($elements->length == 1) {
                        ...
                        if (strpos ($fieldType, "Peron\\AmazonMwsFullfillment\\") === false)
                            $fieldType = 'Peron\\AmazonMwsFullfillment\\'.$fieldType;
            ...
            if (is_array($fieldType)) {
                if ($this->_isComplexType($fieldType[0])) {
                    if (array_key_exists($fieldName, $array)) {
                        ...
                        if (count ($elements) >= 1) {
                            //require_once (dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . str_replace('_', DIRECTORY_SEPARATOR, $fieldType[0]) . ".php");
                            if (!strpos ($fieldType[0], "Peron\\AmazonMwsFullfillment\\"))
                                $fieldType[0] = 'Peron\AmazonMwsFullfillment\\'.$fieldType[0];
                            foreach ($elements as $element) {
                                $this->_fields[$fieldName]['FieldValue'][] = new $fieldType[0]($element);
                            }
                 ...
                 if ($this->_isComplexType($fieldType)) {
                    if (array_key_exists($fieldName, $array)) {
                        //require_once (dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . str_replace('_', DIRECTORY_SEPARATOR, $fieldType) . ".php");
                        if (!strpos ($fieldType, "Peron\\AmazonMwsFullfillment\\"))
                            $fieldType = 'Peron\AmazonMwsFullfillment\\'.$fieldType;
